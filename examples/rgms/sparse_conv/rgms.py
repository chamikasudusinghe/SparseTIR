...
def ell3d_fp16(d0: int, d1: int, d2: int, nnz: int, nnz_rows: int, nnz_cols: int, feat_out: int):
    @T.prim_func
    def func(
        a: T.handle,
        wx: T.handle,
...
def rgms_tensorcore(
    ...
    mod = tvm.IRModule.from_expr(
        rgcn_hetero_forward(m, n, num_rels, feat_in, feat_out).with_attr("horizontal_fuse", True)
    )
